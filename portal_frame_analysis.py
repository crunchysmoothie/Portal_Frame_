...
def import_data(file):
    with open(file) as f:
        ...
        node_loads = {node_load['node']: {'direction': node_load['direction'], 'magnitude': node_load['magnitude']}
                      for node_load in data.get('nodal_loads', [])}

        member_loads = {member_load['member']: {'direction': member_load['direction'],
                                                'w1': member_load['w1'], 'w2': member_load['w2']}
...
# Create a new model
    ...
    for support in data['supports']:
        frame.def_support(support, data['supports'][support]['DX'], data['supports'][support]['DY'],)
    ...
    for node_load in data['nodal_loads']:
        frame.add_node_load(node_load, data['nodal_loads'][node_load]['direction'], data['nodal_loads'][node_load]['magnitude'])
